...
import nettion.features.module.modules.render.Animations;
...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                final boolean auraBlock = (Objects.requireNonNull(Nettion.instance.getModuleManager().getModuleByClass(Killaura.class).isEnabled())) && (Killaura.blockmode.getValue() != Killaura.BlockMode.None) && Killaura.target != null;
                ...
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            if(Nettion.instance.getModuleManager().getModuleByClass(Animations.class).isEnabled()) {
                                ...
                                if(Animations.mode.getValue()==Animations.renderMode.Autumn){
                                    float var92 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI);
                                    this.transformFirstPersonItem(f / 2.0f, 0.0f);
                                    GL11.glRotatef((float) (-var92 * 40.0f / 2.0f), (float) (var92 / 2.0f), (float) -0.0f, (float) 9.0f);
                                    GL11.glRotatef((float) (-var92 * 30.0f), (float) 1.0f, (float) (var92 / 2.0f), (float) -0.0f);
                                    this.doBlockTransformations();
                                }
                                if(Animations.mode.getValue()==Animations.renderMode.Crazy){
                                    this.transformFirstPersonItem(f / 2.0f, 0.0f);
                                    if (rot < 360) {
                                        rot += 5;
                                    } else {
                                ...
                                if(Animations.mode.getValue()==Animations.renderMode.External){
                                    this.transformFirstPersonItem(f, 0.0F);
                                    this.doBlockTransformations();
                                ...
                                if(Animations.mode.getValue()==Animations.renderMode.LiquidBounce) {
    ...
    private void jello(float swingProgress) {
        float smooth = (swingProgress * 0.8f - (swingProgress * swingProgress) * 0.8f);
        GlStateManager.translate(0.7, -0.4F, -0.8F);
        GlStateManager.rotate(40.0F, -0.1f, 2 + smooth * 5f, smooth * 5);
