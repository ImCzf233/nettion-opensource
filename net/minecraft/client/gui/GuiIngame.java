...
import nettion.features.module.modules.render.Hotbar;
import nettion.ui.fonts.old.Fonts;
import nettion.ui.notification.Notification;
import nettion.utils.animation.SimpleAnimation;
...
public class GuiIngame extends Gui
{
    ...
    nettion.features.module.modules.render.Scoreboard scoreboardMod = new nettion.features.module.modules.render.Scoreboard();
    ...
    public void bloom() {
        ...
        if (scoreobjective1 != null) {
            if (scoreboardMod.ena.getValue()) {
                this.renderScoreboard2(scoreobjective1, scaledresolution);
            }
        }
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        HUD hud = (HUD) ModuleManager.getModuleByClass(HUD.class);
        hud.blurScreen();
        EventBus.getInstance().call(new EventRender2D(partialTicks));
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = this.zLevel;
            this.zLevel = -90.0F;
            simpleAnimation.setAnimation(entityplayer.inventory.currentItem * 20, Hotbar.speed.getValue());
            int itemX;
            if (Hotbar.animation.getValue()) {
                itemX = (int) (i - 91 + (simpleAnimation.getValue()));
            } else {
                itemX = i - 91 + (entityplayer.inventory.currentItem * 20);
            }
            if(ModuleManager.getModuleByClass(Hotbar.class).isEnabled()) {
                //this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22, 0, 0, 182, 22);
                //this.drawTexturedModalRect(i - 91 - 1 + (simpleAnimation.getValue()), sr.getScaledHeight() - 22 - 1, 0, 22, 24, 22);

                drawRect3(i - 91, sr.getScaledHeight() - 22, i + 91, sr.getScaledHeight() + 22, new Color(20, 20, 20, 50).getRGB());
                drawRect3(itemX, sr.getScaledHeight() - 22, itemX + 22, sr.getScaledHeight(), new Color(230, 230, 230, 180).getRGB());
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        if (ModuleManager.getModuleByClass(nettion.features.module.modules.render.Scoreboard.class).isEnabled()) {
            ...
            int i = Fonts.PF16.getStringWidth(objective.getDisplayName());

            for (Score score : collection)
            {
                ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
                String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.RED + score.getScorePoints();
                i = Math.max(i, Fonts.PF16.getStringWidth(s));
            }

            int i1 = collection.size() * Fonts.PF16.getHeight();
            ...
            for (Score score1 : collection)
            {
                ...
                int k = j1 - j * Fonts.PF16.getHeight();
                int l = scaledRes.getScaledWidth() - k1 + 2;
                drawRect3(l1 - 2, k, l, k + Fonts.PF16.getHeight(), 1342177280);
                RenderUtils.resetColor();
                if (j == collection.size())
                {
                    String s3 = objective.getDisplayName();
                    drawRect3(l1 - 2, k - Fonts.PF16.getHeight() - 1, l, k - 1, 1342177280);
                    RenderUtils.resetColor();
                    drawRect3(l1 - 2, k - 1, l, k, 1342177280);
                    RenderUtils.resetColor();
                    Fonts.PF16.drawString(s3, l1 + i / 2f - Fonts.PF16.getStringWidth(s3) / 2f, k - Fonts.PF16.getHeight() + 0.8f, Color.WHITE.getRGB());
                }
                Fonts.PF16.drawString(s1, l1, k, Color.WHITE.getRGB());
                if (!scoreboardMod.noPoint.getValue()) {
                    Fonts.PF16.drawString(s2, l - Fonts.PF16.getStringWidth(s2), k, Color.WHITE.getRGB());
                }
            }
        } else {
            Scoreboard scoreboard = objective.getScoreboard();
    ...
    private void renderScoreboard2(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score ignored : collection)
        {
            ...
            int k = j1 - j * Fonts.PF16.getHeight();
            int l = scaledRes.getScaledWidth() - k1 + 2;
            drawRect3(l1 - 2, k, l, k + Fonts.PF16.getHeight(), new Color(0, 0, 0, 255).getRGB());
            RenderUtils.resetColor();
            if (j == collection.size())
            {
                drawRect3(l1 - 2, k - Fonts.PF16.getHeight() - 1, l, k - 1, new Color(0, 0, 0, 255).getRGB());
                RenderUtils.resetColor();
                drawRect3(l1 - 2, k - 1, l, k, new Color(0, 0, 0, 255).getRGB());
                RenderUtils.resetColor();
            }
        }
    }
